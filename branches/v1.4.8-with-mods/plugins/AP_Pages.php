...
if (isset($_POST['save_page'])) {
	...
	if (strlen($alias) == 0 || strlen($uri) == 0) {
		...
		$db->query("UPDATE `{$db_prefix}pages` SET `uri`='{$uri}', `template`='{$template}', `editor_id`={$pun_user['id']}, `edited`={$now} WHERE `alias`='{$alias}'") or error('Unable to update users', __FILE__, __LINE__, $db->error());
		if (!$db->affected_rows())
			$db->query("INSERT INTO `{$db_prefix}pages`(`alias`, `uri`, `template`, `editor_id`, `edited`) VALUES('{$alias}', '{$uri}', '{$template}', {$pun_user['id']}, {$now})") or error('Unable to insert into users', __FILE__, __LINE__, $db->error());
...
<?php if (isset($row['subject'])): ?>
						<strong><a href="<?php echo $row['alias'] ?>"><?php echo htmlspecialchars($row['subject']) ?></a></strong><br />
<?php endif; ?>
						<a href="<?php echo $row['alias'] ?>"><?php echo $row['alias'] ?></a> &rarr;
						<?php echo $row['uri'] ?>
